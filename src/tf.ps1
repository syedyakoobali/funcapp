...
$TerrafomMinimumVersion = [version]"0.13.5"
...
function LockdownTerraformBackend {
    ...
    Write-Verbose "[Terraform State] Set storage account firewall to `"default-action: Allow`"..."
    az storage account update --name $global:TfStateStorageAccountName --default-action "Allow" --output none
    if ($LastExitCode -gt 0) { throw "az CLI error." }
}
